...
def sample_estimator(model, num_classes, feature_list, train_loader):
    ...
    model.eval()
    group_lasso = sklearn.covariance.EmpiricalCovariance(assume_centered=False)     # get covariance
    correct, total = 0, 0
    num_output = len(feature_list)
    num_sample_per_class = np.empty(num_classes)        # make empty array size num_class
...
def get_Mahalanobis_score(model, test_loader, num_classes, outf, out_flag, net_type, sample_mean, precision, layer_index, magnitude):
    ...
    for data, target in test_loader:
        data, target = data.cuda().requires_grad_(True), target.cuda()

        out_features = model.intermediate_forward(data, layer_index)
        out_features = out_features.view(out_features.size(0), out_features.size(1), -1)
        out_features = torch.mean(out_features, 2)      # second dimenstion
...
def get_posterior(model, net_type, test_loader, magnitude, temperature, outf, out_flag):
    ...
    total = 0
    if out_flag == True:
        temp_file_name_val = '%s/confidence_PoV_In.txt'%(outf)          # test_loader -> PoV_In (in-distribution)
        temp_file_name_test = '%s/confidence_PoT_In.txt'%(outf)
    else:
        temp_file_name_val = '%s/confidence_PoV_Out.txt'%(outf)         # out_test_loader -> PoV_Out (out-of-distribution)
    ...
    for data in test_loader:
        total += data['input'].size(0)
        data = data['input'].cuda().requires_grad_(True)
        ...
        # Normalizing the gradient to binary in {0, 1}                  # adversarial perturbation (ODIN)
        gradient =  torch.ge(data.grad.data, 0)
        gradient = (gradient.float() - 0.5) * 2
        if net_type == 'densenet':
            gradient.index_copy_(1, torch.LongTensor([0]).cuda(), gradient.index_select(1, torch.LongTensor([0]).cuda()) / (63.0/255.0))
        ...
        outputs = outputs / temperature                                 # temperature softmax
        soft_out = F.softmax(outputs, dim=1)
        soft_out, _ = torch.max(soft_out.data, dim=1)
        
        for i in range(data.size(0)):
            if total <= 2:                                            # validation number minimum value is 2
                g.write("{}\n".format(soft_out[i]))
            else:
                f.write("{}\n".format(soft_out[i]))
...
def get_generalized_posterior(model, net_type, test_loader, magnitude, temperature, outf, out_flag):
    '''
    Compute the maximum value of (processed) posterior distribution - ODIN
    return: null
    '''
    ...
    for data, _ in test_loader:
        ...
        tempInputs = torch.add(data.data,  -magnitude, gradient)
        with torch.no_grad():
            g_x, h_x = model(tempInputs)
        soft_out = h_x

        # outputs = outputs / temperature                                 # temperature softmax
        # soft_out = F.softmax(soft_out, dim=1)
        soft_out, _ = torch.max(soft_out.data, dim=1)
